...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Sequen\n' // GANTI NAMA LU
            + 'ORG:OWNER BOTZ;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6289689542663:+62 896-8954-2663\n' // GANTI NOMOR LU
            + 'END:VCARD'
numbernye = '0'
fake = '*Sequen Bot*'
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	...
	baby.on('connecting', () => {
		start('2', 'Sequen Bot Connecting...')
	})
	baby.on('open', () => {
		success('2', 'Sequen Bot Actived')
	})
...
*/
	...
	case 'randomhentai':
				...
				baby.sendMessage(from, `Sedang Dikirim...!`, MessageType.text, selepbot)
				anu = await fetchJson(`https://tobz-api.herokuapp.com/api/hentai?apikey=BotWeA`, { method: 'get' })
				buffer = await getBuffer(anu.result)
				baby.sendMessage(from, buffer, image, { caption: `RANDOM HENTAI!`, quoted: mek })
				break
	...
	case 'joox':
					   ...
					   if (data.error) return reply(data.error)
						...
						baby.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${data.result.title}.mp3`, quoted: mek})
						await limitAdd(sender)
						break				
			case 'runtime':
...
*/
		...
		case 'nsfwneko':
			if (!isNsfw) return reply(nad.nsfwoff())
				...
				buffer = await getBuffer(anu.result)
				baby.sendMessage(from, buffer, image, { caption: `NSFW NEKO!`, quoted: mek })
				break
			case 'nsfwtrap':
					var itsme = `${numbernye}@s.whatsapp.net`
					var split = `${fake}`
					...
					buffer = await getBuffer(anu.result)
					baby.sendMessage(from, buffer, image, { caption: `NSFW TRAP!`, quoted: mek })
					break	
			case 'nsfwblowjob':
				if (!isNsfw) return reply(nad.nsfwoff())
						var itsme = `${numbernye}@s.whatsapp.net`
						...
						anu = await fetchJson(`https://tobz-api.herokuapp.com/api/nsfwblowjob?apikey=BotWeA`, { method: 'get' })
						buffer = await getBuffer(anu.result)
						baby.sendMessage(from, buffer, image, { caption: `BLOWJOB!`, quoted: mek })
						break
				case 'waifu':
							var itsme = `${numbernye}@s.whatsapp.net`
							var split = `${fake}`
								...
								imageToBase64(res.data.img)
									.then(
										(ress) => {
											...
											baby.sendMessage(from, 'Sedang Dikirim...!', MessageType.text, selepbot)
											baby.sendMessage(from, buf, MessageType.image)
										})
							})
							break			
                case 'kpop':
